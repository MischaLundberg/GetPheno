...
__version__ = "V1.26_Alpha-20.12.2025"
disclaimer_text = (
...
def format_numeric(entry, Mode):
    ...
    decimal_formatted = decimal_part.ljust(2, "0") if eM and decimal_part else decimal_part
...
def main(lpr_file, pheno_request, stam_file, addition_information_file, use_predefined_exdep_exclusions, general_exclusions, diagnostic_col, 
    ...
    if (load_precreated_phenotypes):
        ...
        if verbose:
            ...
            usage()
        # Load STAM file (-i)
        # df3 = load_stam_file(stam_file=stam_file, sep=isep, birthdatecol=birthdatecol, diagnostic_col=diagnostic_col, sexcol = sexcol, stam_cols_to_read_as_date = stam_cols_to_read_as_date)
        df3 = load_stam_file(stam_file=stam_file, sep=isep, birthdatecol=birthdatecol, diagnostic_col=diagnostic_col, sexcol = sexcol, stam_cols_to_read_as_date = stam_cols_to_read_as_date, iidcol=iidcol)
        ...
        if verbose:
            ...
            logger.info("[main] Mem after loading stam_file input:")
            usage()
        ...
        if (addition_information_file != ''):
            df4 = load_stam_file(stam_file=addition_information_file, sep=jsep, birthdatecol=birthdatecol, diagnostic_col=diagnostic_col, sexcol = sexcol, stam_cols_to_read_as_date = stam_cols_to_read_as_date, iidcol=iidcol)
        ...
        if h5_exist and not BuildEntryExitDates:
            ...
            if not lowMem:
                ...
                first_write = True  # Control whether to overwrite or append to the file
                df3backup = df3.copy()
                iidsbackup = df3backup[iidcol].unique() 
                
                # Convert to list to ensure consistent indexing
                iids_list = list(iidsbackup)
                batch_size = int(batchsize)
                num_batches = int(np.ceil(len(iids_list) / batch_size))
                ...
                for batch_num in range(num_batches):
                    ...
                    iid_batch = iidsbackup[start_idx:end_idx]
            ...
            for batch_num in range(num_batches):
                ...
                df1 =  batch_load_lprfile(df = df1, lprfile = lpr_file, lpr_recnummer = lpr_recnummer, lpr2nd_file = lpr2nd_file, lpr2nd_recnummer = lpr2nd_recnummer,
                            ...
                            potential_lpr_cols_to_read_as_date = potential_lpr_cols_to_read_as_date, lpr_cols_to_read_as_date = lpr_cols_to_read_as_date,
                            verbose = verbose, dta_input = dta_input, DateFormat = DateFormat, diagnostic_col = diagnostic_col, diagnostic2nd_col = diagnostic2nd_col,
                            filter_on_diagcol = True, diags = flattened_pheno_requests) #diags = in_pheno_codes)
                gc.collect()
                ...
                logger.info(f"[main] Batch {batch_num + 1}: Loaded {len(df1)} rows for {df1[iidcol].nunique()} unique IIDs")
...
def process_pheno_and_exclusions(MatchFI, df3, df1, iidcol, verbose, ctype_excl, ctype_incl, ctype_col, Filter_YoB, Filter_Gender, use_predefined_exdep_exclusions, cluster_run, 
                                 ...
                                 input_date_out_name, append, df4, icdprefix, noLeadingICD, lifetime_exclusions, oneYearPrior_exclusions, post_exclusions, covariates, lowMem, main_pheno_name):
    ...
    if (qced_iids != ""):
        try:
            ...
            if verbose:
                logger.info("[process_pheno_and_exclusions] Assuming the first column contains the IID")
    ...
    if (general_exclusions != ""):
        try:
            ...
            if verbose:
                logger.info("[process_pheno_and_exclusions] Mem after building general exclusions and excluding these from the base file:")
                usage()
        except Exception as e:
    ...
    if (ATC_Requested == "All" or ATC_Requested == "Some"):
        if (atc_file != ""):
            ...
            atc_df1 = load_stam_file(stam_file=atc_file, sep=sep, birthdatecol="", diagnostic_col=atc_diag_col, sexcol = "", stam_cols_to_read_as_date = atc_cols_to_read_as_date, iidcol=iidcol)
    ...
    if append:
        ...
        if os.path.exists(outfile):
            ...
            # add any missing columns expected by the file
            for c in existing_cols:
                if c not in final_df.columns:
                    final_df[c] = ""
                    final_df = final_df.copy()
            # add any new columns to the header order
    ...
    base_cols = [iidcol, 'diagnosis', 'diagnoses', 'first_dx', 'in_dates', 'out_dates', 'birthdate', 'Age_FirstDx']
    if ctype_col in final_df.columns:
        base_cols.insert(base_cols.index('Age_FirstDx'), ctype_col)
    ...
    def disorder_names(df):
        return df['Disorder'].tolist() if not df.empty else []

    phenos = list(in_pheno_codes['Disorder']) if isinstance(in_pheno_codes, pd.DataFrame) else []
    excl_names = disorder_names(lifetime_exclusions) + disorder_names(oneYearPrior_exclusions) + disorder_names(post_exclusions) + disorder_names(covariates)

    def exp_cols_for(names):
        cols = []
        for d in names:
            cols += [d, f"{d}_Codes", f"{d}_In_Dates", f"{d}_Out_Dates", f"{d}_earliest_date", f"{d}_latest_date",
                    f"{d}_n_diags", f"{d}_n_unique_in_days"]
            for extra in extra_cols_to_keep:
                cols.append(f"{d}_{extra}")
        return cols

    expected_cols = list(dict.fromkeys(base_cols + exp_cols_for(phenos + excl_names)))

    # Add Exclusion “inflicted changes” columns when exclusions are present
    if not (lifetime_exclusions.empty or oneYearPrior_exclusions.empty or post_exclusions.empty):
        excls = []
        for df in (lifetime_exclusions, oneYearPrior_exclusions, post_exclusions):
            if not df.empty:
                excls += df['Disorder'].tolist()
        for ex in excls:
            expected_cols += [
    ...
    # Add Level2/Level3 only if you actually ran exclusions
    if not (lifetime_exclusions.empty or oneYearPrior_exclusions.empty or post_exclusions.empty):
        expected_cols += [
            'diagnoses_Level2_modifier','disorder_Level2_modifier','date_Level2_modifier',
            'Level2_diagnoses','Level2_dates','Level2_ExclusionReason','Level2_FirstDx','Level2_AgeExclusion',
            'Level3_Age_FirstDx','Level3_CaseControl','Level3_CaseControl_AgeExclusions',
            'Level3_Sankey','Level3_Sankey_data','Level3_Sankey_source','Level3_Sankey_target','Level3_Sankey_value'
        ]


    # If appending to an existing file, preserve its header order
...
def load_stam_file(
    ...
    for path in paths:
        if dta_input:
            ...
            if date_cols:
                logger.info(f"[load_stam_file] Original cols: {requested_dates}")
                logger.info(f"[load_stam_file] date columns in {path}: {date_cols}")
    ...
    if iidcol is not None and iidcol in df3.columns:
        try:
            df3[iidcol] = normalize_iid_series(df3[iidcol], target="str")
            if verbose:
                logger.info(f"[load_stam_file] Normalized IID column '{iidcol}' to string")
        except Exception as e:
            logger.info(f"[load_stam_file] Warning: could not normalize IID column '{iidcol}': {e}")
    # End new 02.12.2025
    # renames
    if birthdatecol in df3.columns and birthdatecol != "birthdate":
        df3 = df3.rename(columns={birthdatecol: "birthdate"})
        if verbose:
            logger.info(f"[load_stam_file] Renamed {birthdatecol} → 'birthdate'")

    if diagnostic_col in df3.columns and diagnostic_col != "diagnosis":
        df3 = df3.rename(columns={diagnostic_col: "diagnosis"})
        if verbose:
            logger.info(f"[load_stam_file] Renamed {diagnostic_col} → 'diagnosis'")
...
def process_lpr_data(lpr_file, lpr2nd_file, dta_input, sep, lpr_cols_to_read_as_date, DateFormat, potential_lpr_cols_to_read_as_date, diagnostic_col, diagnostic2nd_col, lpr_recnummer, lpr2nd_recnummer, iidcol):
    ...
    if (len(secondary_paths) > 0):
        for lprfile, lpr2ndfile in zip(file_paths, secondary_paths):
            ...
            #df_lpr = load_stam_file(stam_file=lprfile, sep=sep, birthdatecol="", diagnostic_col="", sexcol = "", stam_cols_to_read_as_date = [lpr_cols_to_read_as_date + potential_lpr_cols_to_read_as_date])
            df_lpr = load_stam_file(stam_file=lprfile, sep=sep, birthdatecol="", diagnostic_col="", sexcol = "", stam_cols_to_read_as_date = lpr_cols_to_read_as_date + potential_lpr_cols_to_read_as_date, iidcol=iidcol)
            #df_lpr2nd = load_stam_file(stam_file=lpr2ndfile, sep=sep, birthdatecol="", diagnostic_col="", sexcol = "", stam_cols_to_read_as_date = [lpr_cols_to_read_as_date + potential_lpr_cols_to_read_as_date])
            df_lpr2nd = load_stam_file(stam_file=lpr2ndfile, sep=sep, birthdatecol="", diagnostic_col="", sexcol = "", stam_cols_to_read_as_date = lpr_cols_to_read_as_date + potential_lpr_cols_to_read_as_date, iidcol=iidcol)
        ...
        for lprfile in file_paths:
            ...
            #df_lpr = load_stam_file(stam_file=lprfile, sep=sep, birthdatecol="", diagnostic_col="", sexcol = "", stam_cols_to_read_as_date = set(lpr_cols_to_read_as_date + potential_lpr_cols_to_read_as_date))
            df_lpr = load_stam_file(stam_file=lprfile, sep=sep, birthdatecol="", diagnostic_col="", sexcol = "", stam_cols_to_read_as_date = lpr_cols_to_read_as_date + potential_lpr_cols_to_read_as_date, iidcol=iidcol)
